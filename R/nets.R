...
# nets2 <- function( y , p=1 , GN=TRUE , CN=TRUE , lambda=stop("shrinkage parameter 'lambda' has not been set") , alpha.init=NULL , rho.init=NULL , verbose=FALSE , algorithm='activeset' , weights='adaptive' , maxiter=100 ){
#   
#   # input check
#   if( !is.data.frame(y) & !is.matrix(y) ){
#     stop("The 'y' parameter has to be a TxN matrix or a data.frame of data")
...
#     stop("At least A or G have to be true")  
#   }
#   if( !(algorithm %in% c('std','shooting','activeset','activeset2')) ){
